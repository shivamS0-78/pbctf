...
EvaluatorSchema.pre('save', async function(this: IEvaluator) {
  ...
  this.evaluatedCount = this.assignedTeams.filter((t: IAssignedTeam) => t.isEvaluated).length;
  if (this.stats) {
...
if (mongoose.models.Evaluator) {
  delete mongoose.models.Evaluator;
}

const Evaluator: Model<IEvaluator> = mongoose.model<IEvaluator>('Evaluator', EvaluatorSchema);
