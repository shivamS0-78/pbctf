  ...
  },
  {
    ...
    logo: (
      <svg
        ...
        className="sponsors__logo sponsors__logo--google"
      >
        <path
...
const TICKER_SET = Array.from({ length: 4 }, () => PARTNERS).flat();
          ...
          <div className="sponsors__ticker-track" aria-hidden="true">
            <div className="sponsors__ticker-inner">
              {TICKER_ITEMS.map((p, i) => (
                <div key={`${p.id}-${i}`} className="sponsors__ticker-item">
                  {/* eslint-disable-next-line @next/next/no-img-element */}
                  <img
                    src={p.src}
                    alt={p.alt}
                    className={`sponsors__ticker-logo sponsors__ticker-logo--${p.id}`}
                    loading="lazy"
                  />
                </div>
              ))}
