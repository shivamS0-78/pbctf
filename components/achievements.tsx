...
const GlitchTitle = memo(({ glitchEffect }: { glitchEffect: boolean }) => {
  ...
  return (
    <div className="relative mb-8 md:mb-20 text-center w-full">
      ...
      >
        <span className="text-[#00FF7F] gradient-text-green">Point Blank's </span><span className="text-[#00FF88] gradient-text">Achievements</span>
      </motion.h2>
      
      {glitchEffect && (
        <>
          <h2 
            className="glitch-copy absolute top-0 left-0 w-full text-5xl md:text-7xl font-dystopian font-bold text-[#00CC70] opacity-70 text-center"
            style={{ clipPath: 'polygon(0 0, 100% 0, 100% 45%, 0 45%)', transform: 'translate(-5px, -5px)' }}
            aria-hidden="true"
          >
            <span className="text-[#00FF7F]">Point Blank's </span><span className="text-[#00CC70]">Achievements</span>
          </h2>
          <h2 
            className="glitch-copy absolute top-0 left-0 w-full text-5xl md:text-7xl font-dystopian font-bold text-[#8CFF00] opacity-70 text-center"
            style={{ clipPath: 'polygon(0 60%, 100% 60%, 100% 100%, 0 100%)', transform: 'translate(5px, 5px)' }}
            aria-hidden="true"
          >
            <span className="text-[#00FF7F]">Point Blank's </span><span className="text-[#8CFF00]">Achievements</span>
          </h2>
        </>
      )}
    </div>
          ...
          >
            ...
            {menuItems.map((item, index) => {
              ...
              return (
                ...
                >
                  <item.icon className={`w-4 h-4 ${selectedCategory === item.id ? 'text-[#8CFF00]' : 'text-gray-400'}`} />
                    ...
                    <div key={item.id} className="mb-3 relative">
                      <motion.button
                        ...
                        className={`w-full px-4 py-4 rounded-lg transition-all duration-300 flex items-center
                          ${isActive 
                            ? 'bg-gradient-to-r from-[#00FF88]/10 to-[#8CFF00]/5 text-white' 
                            : 'hover:bg-white/5 text-gray-400'}`}
                      >
                        <div className={`w-10 h-10 rounded-full flex items-center justify-center mr-4 
                          ${isActive ? 'bg-gradient-to-r from-[#00FF88] to-[#8CFF00]' : 'bg-gray-800'}`}>
                      ...
                      {isActive && (
                        <div className="h-1 bg-gray-800 mt-1 rounded overflow-hidden">
                          <motion.div 
                            ...
                            className="h-full bg-gradient-to-r from-[#00FF88] to-[#8CFF00]"
                          />
                        </div>
                      )}
                    </div>
